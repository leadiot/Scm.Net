...
namespace Com.Scm.Dev.Icon
{
    ...
    {
        private readonly SugarRepository<ScmDevIconDao> _thisRepository;

        /// <summary>
        /// 
        /// </summary>
        /// <param name="thisRepository"></param>
        public ScmResIconService(SugarRepository<ScmDevIconDao> thisRepository)
        {
        ...
        {
            var catDao = await _thisRepository.Change<ScmDevIconCatDao>().GetByIdAsync(0);
            foreach (var cat in list)
            {
                if (catDao == null)
                {
                    catDao = await _thisRepository.Change<ScmDevIconCatDao>().GetByIdAsync(cat.set_id);
                }
                cat.set = catDao.code;
            }
        }
        ...
        /// <returns></returns>
        [HttpGet("{id}")]
        public async Task<ScmDevIconDto> GetAsync(long id)
        {
            return await _thisRepository
                .AsQueryable()
                .Where(a => a.id == id)
                .Select<ScmDevIconDto>()
                .FirstAsync();
        }
        ...
        /// <returns></returns>
        public async Task<bool> AddAsync(ScmDevIconDto model)
        {
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public async Task<bool> UpdateAsync(ScmDevIconDto model)
        {
