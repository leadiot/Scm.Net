...
export default {
	...
	data() {
		return {
			...
			apiObj: this.$API.nasresapps.page,
			param: {
				dir_id: '0',
	...
	methods: {
		...
		status_list(status) {
			this.$SCM.status_list(this, this.$API.nasresapps.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.nasresapps.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.nasresapps.delete, this.selection);
		},
		show_search() {
