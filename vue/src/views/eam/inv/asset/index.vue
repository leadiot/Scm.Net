...
export default {
	...
	data() {
		return {
			tableName: 'eam_inv_asset',
			apiObj: this.$API.eaminvasset.page,
			list: [],
			param: {
			...
			column: [
				...
				{ prop: 'sku_id', label: 'SKU', width: 100 },
				{ prop: 'bat_id', label: '批次数据', width: 100 },
				{ prop: 'codes', label: '唯一码', width: 100 },
				{ prop: 'organize_id', label: '使用部门', width: 100 },
				{ prop: 'user_id', label: '使用人员', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.eaminvasset.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.eaminvasset.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.eaminvasset.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.eaminvasset.delete, this.selection);
		},
		show_search() {
