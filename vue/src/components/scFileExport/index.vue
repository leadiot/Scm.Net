<template>
	...
	<el-drawer v-model="dialog" :title="title" :size="400" direction="rtl" append-to-body destroy-on-close>
						...
						<el-form label-width="100px" label-position="left" style="margin: 10px 0 20px 0;">
							...
							<el-form-item label="文件类型">
								<el-select v-model="formData.fileType" placeholder="请选择文件类型">
									<el-option v-for="item in fileTypes" :key="item" :label="'*.' + item"
										:value="item" />
								</el-select>
							</el-form-item>
							<slot name="form" :formData="formData"></slot>
						</el-form>
						<el-button v-if="async" type="primary" size="large" style="width: 100%;" @click="download"
