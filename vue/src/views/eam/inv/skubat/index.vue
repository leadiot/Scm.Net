...
export default {
	...
	data() {
		return {
			tableName: 'eam_inv_skubat',
			apiObj: this.$API.eaminvskubat.page,
			list: [],
			param: {
			...
			column: [
				...
				{ prop: 'bat_id', label: '批次ID', width: 100 },
				{ prop: 'total', label: '历史总量', width: 100 },
				{ prop: 'valid', label: '当前数量', width: 100 },
				{ prop: 'rd_id', label: '折旧方案', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.eaminvskubat.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.eaminvskubat.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.eaminvskubat.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.eaminvskubat.delete, this.selection);
		},
		show_search() {
