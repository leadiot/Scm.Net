<template>
    ...
    <i :class="getMsClass()" :style="{ 'fontSize': size + 'px', 'color': color }" v-else-if="isMs()">
        {{ getMsName() }}
    </i>
    <i :class="getMiClass()" :style="{ 'fontSize': size + 'px', 'color': color }" v-else-if="isMi()">
        {{ getMiName() }}
    </i>
...
export default {
    ...
    data() {
        return {
            set_list: ['vs', 'sc', 'mi', 'ms', 'mdi'],//图标系列列表
        }
    },
    methods: {
        /**
         * 判断是否为Scm.Net图标
         * @returns {boolean} 是否为Scm.Net图标
         */
        isSc() {
        ...
        isMs() {
            var tmp = this.name.toLowerCase();
            return tmp.startsWith('ms-');
        },
        isMi() {
            var tmp = this.name.toLowerCase();
            return tmp.startsWith('mi-');
        },
        isMdi() {
            var tmp = this.name.toLowerCase();
            return tmp.startsWith('mdi-');
        },
        /**
         * 获取Scm.Net图标类名
         * @returns {string} 图标类名
