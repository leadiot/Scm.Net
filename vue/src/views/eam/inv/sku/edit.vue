...
<el-form-item label="历史数量" prop="total"> 
	<el-input 
...
rd_id:'', 
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.eaminvsku.edit.get(row.id);
		...
		save() {
			this.$refs.formRef.validate(async (valid) => {
				...
				if (this.$SCM.is_valid_id(this.formData.id)) {
					res = await this.$API.eaminvsku.update.put(this.formData);
				} else {
					res = await this.$API.eaminvsku.add.post(this.formData);
				}
