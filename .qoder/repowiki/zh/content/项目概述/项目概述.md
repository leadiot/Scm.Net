...
- [Scm.Addon/IAddon.cs](file://Scm.Addon/IAddon.cs)
</cite>

## 更新摘要
**所做更改**
- 基于 README.md 和 README.en.md 的重大内容扩展，新增详细的项目介绍、架构说明、技术栈、项目结构、设计原则、核心特性、快速开始、演示信息、浏览器支持矩阵、FAQ、截图等内容
- 更新项目简介章节，反映当前的完整项目概述文档管理模式
...
## 引言
Scm.Net 是一款基于 **.NET 10.0** 及 **Vue 3.0** 构架的企业级快速开发框架，专为供应链系统及企业信息化系统而设计，支持异构应用场景需求。框架由作者多年从事电商供应链系统及企业信息化系统的产品与研发经验积累而来，旨在帮助开发者快速搭建完整的开发框架，并满足多场景下的异构应用场景需求。

该框架已在多个实际产品中得到验证，包括 **OMS**（订单管理系统）、**WMS**（仓储管理系统）、**TMS**（运输管理系统）、**DMS**（配送管理系统）、**BMS**（计费管理系统）、**YMS**（园区管理系统）、**EAM**（资产管理系统）、**IOT**（物联网管理系统）等。

**更新** 基于 README.md 和 README.en.md 的内容扩展，当前文档已从原有的核心项目信息扩展为完整的项目概述文档，涵盖项目介绍、架构说明、技术栈、项目结构、设计原则、核心特性、快速开始、演示信息、浏览器支持矩阵、FAQ、截图等多个方面。
...
### 架构特点
Scm.Net 采用前后端分离模式，后端基于 .NET 10.0 开发，兼容 .NET 6/7/8/9/10 运行时；前端基于 Vue 3.0 及 Element Plus 开发，支持 i18n 多语言。系统无平台依赖，可直接在多平台（Windows、macOS、Linux、HarmonyOS 等）开发与运行，响应式布局支持多种设备终端（电脑、平板、手机）。

### 后端核心依赖
框架采用现代化的后端技术栈，包括：
- **SqlSugarCore**：ORM 数据访问
- **Newtonsoft.Json**：JSON 序列化
- **ImageSharp**：跨平台图像处理
- **MQTTnet**：MQTT 通讯（客户端 + 内置 Broker）
- **RabbitMQ.Client**：RabbitMQ 消息队列
- **Quartz.NET**：定时任务调度
- **SignalR**：实时通讯
...
graph TB
subgraph "前端技术栈"
A[Vue 3.0] --> B[Element Plus]
A --> C[i18n 多语言]
D[响应式布局] --> E[多设备支持]
end
subgraph "后端技术栈"
F[.NET 10.0] --> G[SqlSugarCore]
F --> H[ImageSharp]
F --> I[MQTTnet]
F --> J[RabbitMQ.Client]
F --> K[Quartz.NET]
...
- [README.en.md:14-33](file://README.en.md#L14-L33)
...
**更新** 基于 README.md 和 README.en.md 的内容扩展，当前文档已从原有的核心项目信息扩展为完整的项目概述文档，涵盖项目介绍、架构说明、技术栈、项目结构、设计原则、核心特性、快速开始、演示信息、浏览器支持矩阵、FAQ、截图等多个方面，为开发者提供了全面的项目理解和使用指导。
