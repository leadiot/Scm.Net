...
## 更新摘要
**所做更改**
- 新增部署架构设计章节，详细说明生产环境三层架构
- 新增环境准备子系统，包含服务器硬件要求和操作系统配置
- 新增应用部署子系统，涵盖启动装配和服务注册流程
- 新增数据库初始化子系统，详细说明SqlSugar配置和脚本执行
- 新增监控配置子系统，包含性能监控和日志配置
- 新增性能调优子系统，提供具体的优化建议和参数配置
- 更新故障排除指南，增加常见问题的诊断和解决方案
...
## 简介
本指南面向生产环境部署 Scm.Net 的工程团队与运维人员，提供完整的部署架构设计、环境准备、应用部署、数据库初始化、监控配置、性能调优等6个子系统的详细技术文档。文档基于仓库中的配置与启动逻辑进行梳理，确保部署步骤与代码实现保持一致。

## 部署架构设计
生产部署建议采用"反向代理 + 应用容器 + 数据库/缓存/消息队列"三层架构。该架构具有高可用性、可扩展性和安全性特点。
