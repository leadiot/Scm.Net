...
<el-form-item label="采集器" prop="reader_id"> 
	<el-input 
		v-model="formData.reader_id" 
		placeholder="请输入采集器" 
		:maxlength="11" 
		show-word-limit 
		clearable 
...
dir:'', 
		...
		async open(row) {
			if (!row || !row.id) {
				...
				this.mode = "edit";
				var res = await this.$API.eamlogsn.edit.get(row.id);
		...
		save() {
			this.$refs.formRef.validate(async (valid) => {
				...
				if (this.$SCM.is_valid_id(this.formData.id)) {
					res = await this.$API.eamlogsn.update.put(this.formData);
				} else {
					res = await this.$API.eamlogsn.add.post(this.formData);
				}
