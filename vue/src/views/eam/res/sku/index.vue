...
export default {
	...
	data() {
		return {
			tableName: 'eam_res_sku',
			apiObj: this.$API.eamressku.page,
			list: [],
			param: {
			...
			column: [
				...
				{ prop: 'codec', label: 'SKU编码', width: 100 },
				{ prop: 'names', label: '系统名称', width: 100 },
				{ prop: 'namec', label: 'SKU名称', width: 100 },
				{ prop: 'cat_id', label: '分类ID', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.eamressku.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.eamressku.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.eamressku.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.eamressku.delete, this.selection);
		},
		show_search() {
