...
export default {
	...
	data() {
		return {
			tableName: 'eam_inv_sku',
			apiObj: this.$API.eaminvsku.page,
			list: [],
            param:{
			...
			column: [
                ...
                { prop: 'spu_id', label: 'SPU', width: 100 },
                { prop: 'sku_id', label: 'SKU', width: 100 },
                { prop: 'total', label: '历史数量', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.eaminvsku.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.eaminvsku.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.eaminvsku.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.eaminvsku.delete, this.selection);
		},
		show_search() {
