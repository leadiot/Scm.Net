...
## 更新摘要
**所做更改**
- 更新文档以反映 Wiki 文档系统的重构：用户认证系统相关的详细文档已被删除，当前仅保留核心认证功能说明
- 简化认证系统文档结构，聚焦核心认证流程和基本组件
- 移除 OTP 验证码认证系统的详细实现说明，保留基本架构概述
- 更新架构图和流程图，反映简化的认证系统结构
- 保持 API 接口定义的完整性，但简化说明文字
...
## 简介
本技术文档面向 Scm.Net 的用户认证体系，聚焦核心认证功能的实现机制与 OperatorService 核心服务的设计与实现。内容涵盖：
- 核心认证方式：密码认证、OTP 验证码（手机/邮箱）、社交登录（OAuth/OIDC）、令牌认证等
...
**重要说明**：由于 Wiki 文档系统的重构，本文档已简化为仅保留核心认证功能说明，详细的 OTP 实现文档已被移除。
...
- OTP 登录：按手机号、邮箱进行验证码校验；若 auto=true 且用户不存在则自动注册
...
- ScmLoginModeEnum：统一登录模式枚举，覆盖 ByPass、ByPhone、ByEmail、ByOauth、ByOidc 等
...
## 结论
Scm.Net 的认证体系以 OperatorService 为核心，结合基本的 OTP 与 OIDC 等多种认证方式，配合 JWT 中间件与注册策略，形成统一、可扩展、可审计的认证框架。通过合理的配置与安全加固，可在保障用户体验的同时满足企业级安全要求。由于文档系统重构，本文档已简化为仅保留核心认证功能说明，详细的 OTP 实现文档已被移除。
