...
export default {
	name: 'eambatruleheader',
	components: {
		edit: defineAsyncComponent(() => import("./edit")),
	},
	data() {
		return {
			tableName: 'eambatruleheader',
			apiObj: this.$API.eambatruleheader.page,
			list: [],
			param: {
			...
			column: [
				...
				{ prop: 'def_value', label: '默认数据', width: 100 },
	...
	methods: {
		...
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.eambatruleheader.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.eambatruleheader.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.eambatruleheader.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.eambatruleheader.delete, this.selection);
		},
		show_search() {
