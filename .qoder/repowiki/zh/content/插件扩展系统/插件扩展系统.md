...
## 更新摘要
**所做更改**
- 新增插件架构设计章节，详细说明整体架构理念
- 新增音频插件系统完整技术文档，包含格式支持、元数据处理
- 新增图像插件系统完整技术文档，包含多帧处理、特效功能
- 新增视频插件系统完整技术文档，包含接口设计与扩展点
- 新增自定义插件开发完整指南，包含最佳实践和安全考虑
...
- [Scm.Plugin/IPlugin.cs:6-11](file://Scm.Plugin/IPlugin.cs#L6-L11)
- [Scm.Plugin/PluginType.cs:3-11](file://Scm.Plugin/PluginType.cs#L3-L11)
...
### 接口设计与功能特性
音频插件系统基于 IPluginAudio 接口设计，提供完整的音频文件处理能力：

- **格式检测**：支持扩展名级别的可读/可写文件判断
- **元数据提取**：提供音频文件的基本信息提取能力
- **格式支持**：涵盖主流音频格式的处理需求
...
**图表来源**
- [Scm.Plugin.Audio/IPluginAudio.cs:3-8](file://Scm.Plugin.Audio/IPluginAudio.cs#L3-L8)
- [Scm.Plugin.Audio/ScmAudioFormat.cs:3-10](file://Scm.Plugin.Audio/ScmAudioFormat.cs#L3-L10)
- [Scm.Plugin.Audio/ScmAudioMeta.cs:5-25](file://Scm.Plugin.Audio/ScmAudioMeta.cs#L5-L25)
...
### 元数据处理机制
ScmAudioMeta 提供标准化的音频元数据结构，包括文件基本信息和格式标识。
...
### 多层次接口设计
图像插件系统采用多层次接口设计，提供从基础到高级的完整功能：

- **IPluginImage**：插件接口，定义图像处理能力
- **IImage**：图像对象接口，定义图像基本属性
- **IFrame**：帧接口，支持多帧图像处理
- **ScmImage**：抽象基类，提供丰富的图像处理方法

```mermaid
classDiagram
class IPluginImage {
<<interface>>
+IsImageFile(exts : string) bool
+IsReadableFile(exts : string) bool
+GetReadableExts() FileExt[]
+IsWritableFile(exts : string) bool
+GetWritableExts() FileExt[]
+Load(file : string) IImage
+Load(stream : Stream, format : ScmImageFormat) IImage
+Convert(format : string) bool
+Thumbnail(file : string) IImage
+GetImageInfo(stream : Stream) ScmImageMeta
+Save(file : string, format : ScmImageFormat) bool
+Save(stream : Stream, format : ScmImageFormat) bool
}
class IImage {
<<interface>>
+int Count
...
+string Name
+string Format
+GetFrame(index : int) IFrame
+GetDefaultFrame() IFrame
}
class ScmImage {
<<abstract>>
+Type : PluginType.Image
+Read(file : string) bool
+Read(stream : Stream) bool
+Read(uri : Uri) bool
+Save(file : string) bool
+Save(stream : Stream) bool
+Convert(format : string) bool
+Convert(format : ScmImageFormat) bool
+Resize(width : int, height : int) void
+Scale(scaleX : double, scaleY : double) void
+Rotate(degrees : int) void
+Flip(option : FlipOption) void
+Flop(option : FlopOption) void
+Crop(x : int, y : int, width : int, height : int) void
+GetImageColor(count : int, delta : int) ImageColor[]
+Show() void
+Suspend() void
+Restart() void
+Stop() void
...
**图表来源**
- [Scm.Plugin.Image/IPluginImage.cs:8-89](file://Scm.Plugin.Image/IPluginImage.cs#L8-L89)
- [Scm.Plugin.Image/IImage.cs:3-41](file://Scm.Plugin.Image/IImage.cs#L3-L41)
- [Scm.Plugin.Image/ScmImage.cs:12-233](file://Scm.Plugin.Image/ScmImage.cs#L12-L233)
...
**图表来源**
- [Scm.Plugin.Video/IPluginVideo.cs:3-8](file://Scm.Plugin.Video/IPluginVideo.cs#L3-L8)
- [Scm.Plugin/PluginType.cs:3-11](file://Scm.Plugin/PluginType.cs#L3-L11)
