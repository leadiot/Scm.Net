...
export default {
	...
	data() {
		let defTab = { label: '国别',  'id': '1000000000000000002' };
		return {
			tableName: "scm_sys_region",
			param: {
				key: "",
			},
			selection: [],
	...
	mounted() {
		this.listRegion(this.currTab.id);
	},
	methods: {
		complete() {
		...
		async sort(m) {
			await this.$API.scmadmregion.sort.post(m);
			this.$refs.table.refresh();
		},
		async status_item(e, row) {
			this.$SCM.status_item(this, this.$API.scmadmregion.status, row, row.row_status);
		},
		status_list(status) {
			this.$SCM.status_list(this, this.$API.scmadmregion.status, this.selection, status);
		},
		async delete_item(row) {
			this.$SCM.delete_item(this, this.$API.scmadmregion.delete, row);
		},
		delete_list() {
			this.$SCM.delete_list(this, this.$API.scmadmregion.delete, this.selection);
		},
		open_dialog(row) {
		...
		async listRegion(pid) {
			var res = await this.$API.scmadmregion.list.get(pid);
			this.list = res.data;
		},
		open_newtab(row) {
			let tab = {
